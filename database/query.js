module.exports = app => {
  const statusType = (status, userid) =>
    status.length > 1
  ...
  const avatarQueries = {
    getAvatarToken: `SELECT token,userid FROM avatars WHERE userid=$1 AND`
  };
  ...
  const planQueries = {
    ...
    getAllPlans: (userid, limit, offset, status) =>
      `SELECT * FROM plans WHERE status='${status}' AND userid='${userid}' ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`,
  ...
  const promoQueries = {
    ...
    getAllPromotions: (userid, limit, offset, status) =>
      `SELECT * FROM promotionals WHERE status='${status}' AND userid='${userid}' ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`,
  ...
  const subQueries = {
    ...
    getSomeSubcribers: (userid, limit, offset, status) =>
      `SELECT * FROM subscribers ${statusType(
        status,
        userid
  ...
  const templateQueries = {
    ...
    getSomeTemplates: (userid, limit, offset, status) =>
      `SELECT * FROM templates ${statusType(
        status,
        userid
      )} ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`,
  ...
  const transactQueries = {
    ...
    getSomeTransactactions: (userid, limit, offset, status) =>
      `SELECT * FROM transactions ${statusType(
        status,
        userid
      )} ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`,
