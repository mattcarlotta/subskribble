module.exports = app => {
	...
	const adminQueries = { setUserAsAdmin: "UPDATE users SET isGod=true WHERE id=$1" }

	const avatarQueries = { getAvatarToken: `SELECT token,userid FROM avatars WHERE userid=$1 AND` }

	const authQueries = {
		createNewUser: `INSERT INTO users(email, password, firstName, lastName, company, token, startDate) VALUES ($1, $2, $3, $4, $5, $6, $7)`,
	...
	const dashboardQueries = {
		getChargesSum: `SELECT count(chargeDate) FROM transactions WHERE userid=$1 AND chargeDate>=$2 AND chargeDate<$3`,
		getInactiveSubscriberSum: `SELECT count(status) FROM subscribers WHERE userid=$1 AND status='inactive' OR status='suspended'`,
		getInactiveTemplateSum: `SELECT count(templateName) FROM templates WHERE userid=$1 AND status='suspended'`,
		getMessageSum: `SELECT count(sentDate) FROM messages WHERE userid=$1 AND sentDate>=$2 AND sentDate<$3`,
		getPlanSum: `SELECT count(planName) FROM plans WHERE userid=$1 AND status='active'`,
		getPlansTop6: `SELECT planName,subscribers FROM plans WHERE userid=$1 AND status='active' ORDER BY subscribers DESC LIMIT 6`,
		getPromotionSum: `SELECT count(promoCode) FROM promotionals WHERE userid=$1 AND status='active' AND startDate>=$2 AND startDate<$3`,
		getRefundSum: `SELECT count(refundDate) FROM transactions WHERE userid=$1 AND refundDate>=$2 AND refundDate<$3`,
		getSubscriberSum: `SELECT sum(subscribers) FROM plans WHERE userid=$1 AND startDate>=$2 AND startDate<$3`,
		getTemplateSum: `SELECT count(templateName) FROM templates WHERE userid=$1 AND status='active'`,
	...
	const feedbackQueries = { userFeedback: `INSERT INTO feedback(company, email, reason) VALUES ($1, $2, $3)` }

	const messageQueries = {
		createMessageTransaction: `INSERT INTO messages(userid, template, fromSender, subject, sentDate, plans) VALUES((SELECT id FROM users WHERE id=$1), $2, $3, $4, $5, $6)`,
		deleteOneMessage: `DELETE FROM messages WHERE userid=$1 AND id=$2`,
		getMessageCount: `SELECT count(*) FROM messages WHERE userid=$1`,
		getSomeMessages: `SELECT * FROM messages WHERE userid=$1 ORDER BY key DESC LIMIT $2 OFFSET $3`
	}
	...
	const planQueries = {
		createPlan: `INSERT INTO plans(userid, amount, billEvery, planName, description, setupFee, startDate) VALUES((SELECT id FROM users WHERE id=$1), $2, $3, $4, $5, $6, $7)`,
		...
		getPlanCount: `SELECT count(*) filter (where status = 'active' AND userid=$1) AS active, count(*) filter (where status = 'suspended' AND userid=$1) as inactive FROM plans;`,
		...
		updatePlanStatus: `UPDATE plans SET status=$3 WHERE userid=$1 AND id=$2 RETURNING planName`,
	...
	const promoQueries = {
		createPromotion: `INSERT INTO promotionals(userid, amount, maxUsage, discountType, endDate, promoCode, plans, startDate) VALUES((SELECT id FROM users WHERE id=$1), $2, $3, $4, $5, $6, $7, $8)`,
		deleteOnePromotion: `DELETE FROM promotionals WHERE userid=$1 AND id=$2 RETURNING *`,
		...
		getPromotionCount: `SELECT count(*) filter (where status = 'active' AND userid=$1) AS active, count(*) filter (where status = 'suspended' AND userid=$1) AS inactive FROM promotionals;`,
		updatePromotion: `UPDATE promotionals SET amount=$3, maxUsage=$4, discountType=$5, endDate=$6, promoCode=$7, plans=$8, startDate=$9 WHERE userid=$1 AND id=$2`,
		updatePromotionStatus: `UPDATE promotionals SET status=$3 WHERE userid=$1 AND id=$2 RETURNING *`,
		selectPromotionCode: `SELECT promoCode FROM promotionals WHERE userid=$1 AND promoCode=$2`,
		selectPromotionDetails: `SELECT * FROM promotionals WHERE userid=$1 AND promoCode=$2 AND plans@>$3 AND status='active' AND startDate<=$4 AND endDate>=$4`,
	...
	const subQueries = {
		...
		getSomeSubcribers: (userid, limit, offset, status) => (`SELECT * FROM subscribers ${statusType(status, userid)} ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`),
		getAllEmailsByPlan: `SELECT email FROM subscribers WHERE userid=$1 AND planName=$2`,
		getSubscriberCount: `SELECT count(*) filter (WHERE status = 'active' AND userid=$1) AS active, count(*) filter (where status in ('inactive', 'suspended') and userid=$1) as inactive FROM subscribers;`,
		getSubscriberId: `SELECT id FROM subscribers WHERE userid=$1 AND email=$2`,
		updateOneSubscriber: `UPDATE subscribers SET status=$3, enddate=$4 WHERE userid=$1 AND id=$2 RETURNING subscriber,planName`,
		updateSubscriberCredits: `UPDATE subscribers SET credits=credits+$3 WHERE userid=$1 AND id=$2`
	}
	...
	const templateQueries = {
		...
		deleteOneTemplate: `DELETE FROM templates WHERE userid=$1 AND id=$2 RETURNING *`,
		findTemplateById: `SELECT * FROM templates WHERE userid=$1 AND id=$2`,
		findTemplateByName: `SELECT * FROM templates WHERE userid=$1 and uniqueTemplateName=$2`,
		getAllActiveTemplates: `SELECT * FROM templates WHERE status='active' AND userid=$1 ORDER BY key ASC`,
		getSomeTemplates: (userid, limit, offset, status) => (`SELECT * FROM templates ${statusType(status, userid)} ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`),
		getTemplateCount: `SELECT count(*) filter (WHERE status='active' AND userid=$1) AS active, count(*) filter (where status='suspended' and userid=$1) as inactive FROM templates;`,
		updateTemplate: `UPDATE templates SET fromSender=$3, plans=$4, message=$5, subject=$6, templateName=$7, uniqueTemplateName=$8 WHERE userid=$1 AND id=$2 RETURNING templateName,plans`,
		updateTemplateStatus: `UPDATE templates SET status=$3 WHERE userid=$1 AND id=$2 RETURNING templateName,plans`,
	...
	const transactQueries = {
		...
		deleteOneTransactaction: `DELETE FROM transactions WHERE userid=$1 AND id=$2 RETURNING *`,
		getSomeTransactactions: (userid, limit, offset, status) => (`SELECT * FROM transactions ${statusType(status, userid)} ORDER BY key DESC LIMIT ${limit} OFFSET ${offset};`),
		getTransactionCount: `SELECT count(*) filter (where status in ('paid', 'due') AND userid=$1) AS charges, count(*) filter (where status in ('refund', 'credit') AND userid=$1) AS refunds FROM transactions;`,
		refundTransaction: `INSERT INTO transactions(userid, status, planName, email, subscriber, processor, amount, refundDate) VALUES((SELECT id FROM users WHERE id=$1), $2, $3, $4, $5, $6, $7, $8);`,
		selectTransactionById: `SELECT email,planname,processor,subscriber FROM transactions WHERE userid=$1 AND id=$2`
	}
