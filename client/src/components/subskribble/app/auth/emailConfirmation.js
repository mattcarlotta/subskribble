...
const verificationSuccess = userVerified => (
	<Fragment>
		...
		<p className="message">{userVerified} has been verified! Please log in to begin using your account.</p>
...
export default ({ status, userVerified }) => (
				...
				<div className="message-container">
					<div className="validation-result">
						<div className="validation-icon">
							<i className={`material-icons ${status}`}>{userVerified ? 'check_circle' : 'highlight_off'}</i>
						</div>
						{ userVerified ? verificationSuccess(userVerified) : verificationFailure() }
					</div>
				</div>
			</div>
		</div>
