module.exports = app => {
	const { db, query: { createNotification, deleteUserAccount, getAvatarToken, getCurrentUserDetails, getUserPassword, findCompany, findUserByToken, findUserByEmail, updateCompanyName, updateEmailAddress, updateSidebarState, updateUserName, updateUserPassword, userFeedback, verifyEmail }} = app.database;

	const { badCredentials, companyAlreadyExists, emailAlreadyTaken, invalidPassword, invalidToken, missingPasswords, missingSidebarState, missingToken, notUniquePassword, unableLocatePass, unableToRemove } = app.shared.authErrors;

	const { passwordReset, passwordResetSuccess, passwordResetToken, removedAccountSuccess, thanksForReg, updatedAccount, updatedAccountDetails } = app.shared.authSuccess;
	...
	return {
		...
		deleteAccount: async (req, res, done) => {
			...
			try {
				await db.task('delete-account', async dbtask => {
					...
					const validPassword = await bcrypt.compare(suppliedPassword, user.password);
					...
					const deletedUser = await dbtask.result(deleteUserAccount, [req.session.id, email, company]);
		...
		resetPassword: (req, res, done) => passport.authenticate('reset-password', (err, email) => (
			(err || !email) ? sendError(err || 'No user found!', res, done) : res.status(201).json(passwordResetSuccess(email))
		))(req, res, done),
		...
		updateAccount: async(req, res, done) => {
			...
			try {
				await db.task('update-account', async dbtask => {
					const { email: currentEmail, company: currentCompany, firstname: currentFirstName, lastname: currentLastName } = await dbtask.one(getCurrentUserDetails, [req.session.id]);

					if (currentCompany !== updatedCompany) {
						const existingCompany = await dbtask.oneOrNone(findCompany, [updatedCompany]);
					...
					if (currentFirstName !== updatedFirstName || currentLastName !== updatedLastName) {
						await dbtask.none(updateUserName, [req.session.id, updatedFirstName, updatedLastName]);
					...
					if (suppliedPassword || updatedPassword) {
						if (!suppliedPassword || !updatedPassword) return sendError(missingPasswords, res, done);
						...
						const isNotUniquePassword = await bcrypt.compare(updatedPassword, user.password);
					...
					if (currentEmail !== updatedEmail) {
						const existingUser = await dbtask.oneOrNone(findUserByEmail, [updatedEmail]);
						if (existingUser) return sendError(emailAlreadyTaken, res, done);

						await dbtask.none(updateEmailAddress, [req.session.id, updatedEmail, token]);
						...
						await dbtask.none(createNotification, [req.session.id, 'settings', updatedAccountDetails, date]);
						res.status(201).json({ user: {...req.session }, fetchnotifications: true });
					}
				})
			} catch (err) { return sendError(err, res, done) }
		},
