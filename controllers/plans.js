...
const query = {
  ...
  getPlancount: () => (
    `SELECT count(*) filter (where status = 'active') AS active, count(*) filter (where status = 'inactive') as inactive FROM plans;`
  ),
...
module.exports = app => {
  ...
  const _index = async (req, res) => {
    try {
      ...
      const inactiveplans = await db.any(query.getPlans(10, 0, 'inactive'));
