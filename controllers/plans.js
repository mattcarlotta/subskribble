module.exports = app => {
	const { db, query: { createNotification, createPlan, deleteOnePlan, findPlanById, getAllActivePlans, getAllPlans, getPlanCount, updatePlan, updatePlanStatus, selectPlan } } = app.database;
	const { currentDate, parseStringToNum, sendError } = app.shared.helpers;
	const { createPlanFirst, itemAlreadyExists, missingCreationParams, missingDeletionParams, missingQueryParams, missingSelectParams, missingUpdateParams, unableToLocate } = app.shared.errors;
		...
		create: async (req, res, done) => {
			...
			try {
				await db.task('create-plan', async dbtask => {
					const planExists = await dbtask.oneOrNone(selectPlan, [req.session.id, planname])
					if (planExists) return sendError(itemAlreadyExists('plan'), res, done);

					await dbtask.result(createPlan, [req.session.id, amount, billevery, planname, description, setupfee, date]);

					await dbtask.none(createNotification, [req.session.id, 'content_paste', `A new plan: ${planname}, billed at $${amount}/${billevery}, has been created.`, date]);
		...
		deleteOne: async (req, res, done) => {
			...
			try {
				await db.task('delete-plan', async dbtask => {
					const name = await dbtask.result(deleteOnePlan, [req.session.id, req.params.id]);

					await dbtask.none(createNotification, [req.session.id, 'content_paste', `The following plan: ${name.rows[0].planname}, has been deleted.`, date]);
		...
		index: async (req, res, done) => {
			try {
				await db.task('fetch-index-plans', async dbtask => {
					const activeplans = await dbtask.any(getAllPlans(req.session.id, 10, 0, 'active'));
					const inactiveplans = await dbtask.any(getAllPlans(req.session.id, 10, 0, 'suspended'));
		...
		updateOne: async (req, res, done) => {
			...
			try {
				await db.task('update-plan-record', async dbtask => {
					await dbtask.none(updatePlan, [req.session.id, req.params.id, amount, billevery, planname, description, setupfee, trialperiod]);

					await dbtask.none(createNotification, [req.session.id, 'content_paste', `The following plan: ${planname}, has been successfully edited and updated.`, date]);
		...
		updateStatus: async (req, res, done) => {
			...
			try {
				await db.task('update-plan-status', async dbtask => {
					const name = await dbtask.one(updatePlanStatus, [req.session.id, req.params.id, statusType]);

					const message = updateType === "activated" ? "reactivated" : "suspended"
					await dbtask.none(createNotification, [req.session.id, 'content_paste', `The following plan: ${name.planname}, has been ${message}.`, date]);
